...
if [ "$START_DASHBOARD" = true ]; then
    ...
    if [ -d "venv" ]; then
        venv/bin/python3 src/server.py --osc-port 5005 --web-port 8082 > "$LOG_DIR/movement_dashboard.log" 2>&1 &
    else
        python3 src/server.py --osc-port 5005 --web-port 8082 > "$LOG_DIR/movement_dashboard.log" 2>&1 &
    fi
    DASHBOARD_PID=$!
...
echo "  ./scripts/kill-all-services.sh"
echo ""
