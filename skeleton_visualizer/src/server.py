...
def create_app(state: SkeletonState) -> FastAPI:
    ...
    app = FastAPI(title="Skeleton Visualizer")
    ...
    async def index(request: Request):
        return templates.TemplateResponse("skeleton.html", {"request": request})
...
def main():
    parser = argparse.ArgumentParser(description="Skeleton visualizer server")
    parser.add_argument("--host", default="0.0.0.0", help="Host to bind to")
    parser.add_argument("--port", type=int, default=8092, help="Port to bind to")
    parser.add_argument("--osc-port", type=int, default=5007, help="OSC port to listen on")
    ...
    print(f"Skeleton visualizer starting on http://{args.host}:{args.port}")
