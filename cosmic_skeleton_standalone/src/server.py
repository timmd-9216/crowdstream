    ...
    def _process_video(self):
        ...
        while self.running:
            ...
            if results and results[0].keypoints is not None:
                ...
                if results[0].boxes.id is not None:
                    ...
                    if num_tracked != num_keypoints:
                        ...
                        track_ids = list(range(num_keypoints))
                else:
                    track_ids = list(range(num_keypoints))
                    if num_keypoints > 0 and frame_count % 30 == 0:
                        print(f"⚠️ Frame {frame_count}: Tracking failed, {num_keypoints} people detected but no IDs")
            ...
            with self.lock:
                ...
                if frame_count % 30 == 0 and len(self.poses) > 0:
                    print(f"[DEBUG] Frame {frame_count}: {len(self.poses)} poses tracked, IDs: {sorted(self.poses.keys())}")
            ...
            if self.loop and self.clients:
                ...
                if current_time - last_broadcast >= min_broadcast_interval:
                    ...
                    with self.lock:
                        ...
                        # DEBUG: Log broadcast info
                        if frame_count % 30 == 0 and len(self.poses) > 0:
                            print(f"[BROADCAST] Sending {len(payload['poses'])} poses to {len(self.clients)} clients")
